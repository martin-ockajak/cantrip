  ...
  C: Collectible<i64> + FromIterator<i64> + Iterable<Item<'a> = &'a i64> + Clone + PartialEq + Debug + 'a,
{
  ...
  // add
  assert_equal(values.clone().add(4), &[1, 2, 3, 4]);
  ...
  // delete
  assert_equal(values.clone().delete(&2), &[1, 3]);
  assert_equal(values.clone().delete(&4), &[1, 2, 3]);
  ...
  // delete_all
  assert_equal(values.clone().delete_all(&vec![1, 2, 4]), &[3]);
  assert_equal(values.clone().delete_all(&vec![]), &[1, 2, 3]);
